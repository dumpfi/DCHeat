...
class HeizungskachelHTML extends IPSModule
{
    public function Create()
    {
        ...
        // Heizkreise 1 bis 6 registrieren
        for($i=1; $i<=6; $i++) {
    ...
    public function ApplyChanges()
    {
        ...
        // Heizkreis Variablen hinzufügen
        for($i=1; $i<=6; $i++) {
            $vars[] = "C{$i}_State";
            $vars[] = "C{$i}_Temp";
        }
    ...
    private function GetAllValuesAsJSON()
    {
        ...
        // Heizkreis Daten dynamisch sammeln
        $circuits = [];
        for($i=1; $i<=6; $i++) {
    ...
    public function GetVisualizationTile()
    {
        ...
        foreach($configuredCircuits as $index => $cIndex) {
            ...
            <g class="clickable" onclick="openModal(\'modal_circuit_'.$cIndex.'\')" transform="translate(600, '.$yPos.')">
                ...
                <text x="10" y="'.($blockHeight - 10).'" fill="#e67e22" font-size="18" font-weight="bold" font-family="Arial">
...
HTML;
    ...
    // Hilfsfunktion für den Puffer-Popup Code (damit der Hauptteil übersichtlich bleibt)
